...
fun CategoryCard(
    ...
    LazyRow {
        items(categoryList) { it ->
            Card(
                modifier =
                    Modifier.width(150.dp).height(height = 120.dp).clickable {
                        newsViewModel.getNewsArticleWithCategory(it.categoryName, language = "en")
                    },
                ...
                Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                    Image(
                        painter = painterResource(it.categoryImage),
